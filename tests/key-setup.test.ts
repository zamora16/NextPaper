...
const NO_KEY: Settings = { setupDone: false, s2ApiKey: null }
...
const render = (props: {
  ...
  act(async () => {
    root.render(createElement(KeySetup, { settings: NO_KEY, ...props }))
  })
...
describe("settings", () => {
  it("says there is no key when there is none, and offers no removal", async () => {
    await render({
      settings: { setupDone: true, s2ApiKey: null },
      firstRun: false
    })
  ...
  it("shows a saved key only masked", async () => {
    await render({
      settings: { setupDone: true, s2ApiKey: KEY },
      firstRun: false
    })
