...
;(async () => {
  ...
  await page.evaluate(
    async (ref, fill) => {
      ...
      const items = {
        ...
        nextpaper_cache_v12_EXPIRED: { z: "x", cachedAt: 0 },
      ...
      for (let i = 0; i < fill; i++) {
        items["nextpaper_cache_v12_FILL" + i] = { z: "x", cachedAt: Date.now() - (i + 1) * 1000 }
  ...
  check("expired cache entry removed", !keys.includes("nextpaper_cache_v12_EXPIRED"))
  const cacheKeys = has(/^nextpaper_cache_v12_/)
  ...
  check("oldest excess entries evicted", !keys.includes("nextpaper_cache_v12_FILL44"))
