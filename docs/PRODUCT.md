...
| "How do others actually use this paper?" | **How others cite it**: the sentences where citing papers cite it (with the citing paper's title, year and venue), influential citations first. Only sentences that **name the paper** are shown by default; the rest are behind an explicit link with a warning. Loaded when opened, remembered afterwards. |
| "I only want recent / well-cited work" | A *More filters* panel: year from / to (or last 5 / 10 years) and a minimum number of citations. Papers with no year are left out while a year is set. |
...
| "I don't notice new papers" | **Updates (Novedades)**, in its own tab so it does not push your saved papers down: once a day (and on demand) looks for fresh papers related to your latest saves, tags them *Nuevo* with the reason, and puts a count on the toolbar icon. Dismiss one, or clear the whole list (cleared papers are never suggested again); the list is also capped at 40. |
...
- **Citing sentences:** Semantic Scholar has them for only some citing papers (15–50% in our tests), the extraction is noisy, and papers cited with numbers ("[43]") rarely name the paper, so few sentences pass. The API no longer returns the intent (method / result / background), so there is no such filter.
