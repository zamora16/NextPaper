...
8. Clustering: `clusterAuto` (`lib/clustering.ts`) is average-linkage agglomerative clustering on cosine distance; for every state with 2..min(4, ⌊n/2⌋) groups it merges groups smaller than 2 and keeps the one with the best **silhouette score** (deterministic; fewer than 4 papers = one group). It replaced k-means because regrouping after dropping 10% of the papers kept the same groups far better (ARI 0.75 vs 0.44, docs/EVALUATION.md). Labels come from
   `labelClusters` (`lib/keywords.ts`): terms and phrases of up to 3 words taken from the group's **titles**, each in at least half of them (and at least 2) and at least 0.3 more frequent there than in the other groups; a longer phrase replaces the words it contains; acronyms keep their capitals; a term goes to one group only. **Only groups that got a name are shown**, ordered by mean score; the papers of the rest go together into `OTHER_GROUP` (`@other`, *Other related*), last. When no group can be named the result is a single `RELATED_GROUP` list and the popup shows no headings (a label is inferred information: better none than a misleading one).
...
clusters, grouped by the same rule (named groups + Other, or one list).
...
| `components/RelatedTab.tsx` | The *Related* tab: search, *context card* (which paper or topic this is: title, byline, year; the raw reference only until the analysis returns it), filters, sort, timeline toggle, citation bar, *Start here* (compact cards), subtopic groups (heading dot = timeline lane color), skeleton loading, error and empty states. |
...
| `components/CitationButtons.tsx`, `useCopyAction.ts` | `CitationMenu` (the compact citation button of the Related toolbar: it shows the style and opens the style select plus *copy N citations*; copying one paper is on its card), `CitationBar` (Saved tab: copy N joined to the style select, and `.bib`/`.ris`), and the copy phases hook (busy → copied, or "Pulsa para copiar" fallback). |
...
| `nextpaper_unpaywall_quota` | `{day: "YYYY-MM-DD", n}`: lookups made today. At most 100 a day per browser (`UNPAYWALL_DAILY_LIMIT`); cached answers do not count; the card then says *Límite diario* | one key, replaced each day |
...
### Citation sentences (verified 2026-09-21; feature built and removed the same day)
...
| From-scratch hierarchical clustering + silhouette (replaced k-means) | n ≤ 24 vectors; explainable, dependency-free, and real ML is a goal of the project. Chosen by measurement: k-means regrouped too easily (ARI 0.44 under 10% drops), hierarchical 0.75 (docs/EVALUATION.md). |
...
| No "how others cite it", no year / citation / design filters | Built and removed on 2026-09-21. The citing sentences were rarely available and did not help decide what to read; a result is only ~18 papers, so filtering it adds little (sorting is enough) and only about half of the papers have a detected design. Less UI, less code to keep right. |
