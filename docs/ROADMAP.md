...
| F1.5 | ~~Extra filters~~ **DONE 2026-09-21** (year range, min. citations) | ★★ | S | A *More filters* panel (kept out of the way): year from/to with *last 5 / 10 years*, and a minimum number of citations. Papers with no year are left out while a year is set. `Range` in `lib/view.ts`. **Left:** exclude preprints. |
...
| F2.3 | ~~How others cite this paper~~ **DONE 2026-09-21** (sentences only) | ★★★ | M | `lib/citing.ts` + `components/CitedBy.tsx`: the sentences of citing papers where Semantic Scholar found the citation, loaded on demand (one request, cached). **Intents (background / method / result) no longer come back from the API (always empty), so there is no filter by intent** — inventing one would break the precision rule. Because the extraction is noisy, only sentences that **name the paper** (author + year, two authors, or a proper name from its title) show by default; the others are behind an explicit link with a warning. Audited on 5 well-cited papers (see `docs/ARCHITECTURE.md`). |
