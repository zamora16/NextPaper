...
| F0.6 | ~~Measure the ranking~~ **DONE 2026-09-21** | `docs/EVALUATION.md`, `scripts/audit`: 51 seeds, 18 fields, citation-graph proxies (references, bibliographic coupling), bootstrap intervals. Finding: the ranking is ~2.4× random; the 0.02 bonus, the 40-paper shortlist, the 60 + 40 citing papers and the sources need no change; the "% similar" spans only ~0.93–0.97 on the 18 shown. **Left:** human judgments on ~20 seeds (checks the proxy), the display of the similarity, and the stability of the subtopic groups (the stored vectors allow it with no new requests). |
