...
| F0.4 | ~~Surface background failures~~ **DONE 2026-09-20** | `lastError` in `nextpaper_updates`, shown in `UpdatesTab`. |
...
| F0.6 | ~~Measure the ranking~~ **DONE 2026-09-21** | `docs/EVALUATION.md`, `scripts/audit`: 51 seeds, 18 fields, citation-graph proxies (references, bibliographic coupling), bootstrap intervals. Finding: the ranking is ~2.4× random; the 0.02 bonus, the 40-paper shortlist, the 60 + 40 citing papers and the sources need no change; the "% similar" spans only ~0.93–0.97 on the 18 shown, so it does not rank them (kept anyway: it says how close they are to the open paper and differs between analyses); the subtopic groups were unstable (ARI 0.44) so k-means was replaced by hierarchical clustering (0.75) and only nameable groups are shown. **Left:** human judgments on ~20 seeds (checks the proxy). |
...
| F1.3 | ~~Better open-access coverage~~ **DONE 2026-09-21** | ★★ | S | `lib/unpaywall.ts` + the *Buscar PDF* button: Unpaywall by DOI, on click, for papers Semantic Scholar has no PDF for. Only real PDF links count. Host permission `https://api.unpaywall.org/*` (the store listing must justify it). |
| F1.4 | Library upgrades — **mostly done 2026-09-19** | Done: collections (add/remove/filter/delete), **backup/restore JSON**, **import** BibTeX/RIS/DOI/title lists (merge, never overwrites; hostile files sanitized), verified by `scripts/e2e-import.cjs`. **Remaining:** sort inside *Guardados* (text search is done), bulk actions (multi-select → collection/status), rename collection, duplicate detection across differently-identified papers, map BibTeX `note` / RIS `N1` back to notes on import. |
| F1.5 | ~~Extra filters~~ **REMOVED 2026-09-21** | | | A year range and a minimum-citations filter were built, then removed: a result is only ~18 papers, so narrowing it has little value (the sort already covers it). Worth revisiting only if results become much longer. |
| F1.6 | ~~Study-card heuristics~~ **DONE (design + sample) 2026-09-20** | `lib/study.ts`: design (14 categories) and sample size (participants, or *included* studies for reviews), derived at render time (nothing stored, no cache bump). Chips on every card (*Relacionados* and *Guardados*) (a filter by design was built and removed 2026-09-21: only ~half of the papers have a detected design, and a result is only ~18 papers). **Precision over coverage** (see `docs/PRODUCT.md`): it says nothing when the text is ambiguous. Audited by reading the evidence behind every chip on ~1,250 real abstracts in two rounds (the second, unseen set caught errors the first did not; each is a regression test). Coverage: design ~40–55%, sample ~25–30%. Ideas left: population keywords, measures, a sortable table of saved papers → CSV (the free version of A2), filter by minimum *n*. |
| F1.7 | ~~Dark mode, a11y~~ **DONE 2026-09-20** (shortcuts left) | ★ | S | Redesign: colors are CSS-variable tokens with a light and a dark theme that follows the system; tab list with arrow keys and roving tabindex, visible focus ring, labelled icon buttons, `prefers-reduced-motion`. **Left:** global shortcuts (e.g. `/` to search, `j`/`k` through results). |
| F1.8 | ~~English UI (i18n)~~ **DONE 2026-09-20** | | | `lib/i18n` dictionaries (en source of truth, es type-checked against it), language setting, language-neutral stored data, hover hints on the non-obvious controls. Adding a third language = one more dictionary file. |
...
| F2.2 | ~~"Why is this relevant?"~~ **REMOVED 2026-09-21** | The word-level "Coincide en: …" line was built and then removed: it did not help users decide. A full sentence would need the optional AI layer (A1). |
| F2.3 | ~~How others cite this paper~~ **REMOVED 2026-09-21** | | | Built (`lib/citing.ts`, on demand, only sentences that name the paper) and removed: it rarely had anything to show and did not help decide what to read. Facts learned about the API are kept in `docs/ARCHITECTURE.md` §5 in case it is revisited (the intents come back empty, so a method / result filter is not possible). |
...
| F2.7 | ~~Better cluster labels~~ **DONE 2026-09-21** | ★★ | S | `labelClusters` rewritten: terms and phrases (up to 3 words) from the group's **titles**, each in at least half of them and clearly more frequent than in the other groups; acronyms keep their capitals; no honest term → *Group N*. Tuned on 9 real analyses and judged on 10 others (`scripts/audit`). **Left:** the *representative paper* of each group as a chip. |
...
3. ~~F2.1 + F2.2~~ done; F2.3 was built and removed.
